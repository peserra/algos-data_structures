...
// ANTECESSOR
Item *maximoRecursivo(Item *item)
{
    if (item != NULL)
    {
        if (item->dir) // desce recursivamente pelo filho esquerdo
        {
            maximoRecursivo(item->dir);
        }
        else
        {
            return item;
...
/*  Recebe um ponteiro para um item
    Caso exista na arvore, calcula seu antecessor
    Devolve o ponteiro para o sucessor, ou Null caso nao exxista
*/
Item *antecessor(Item *item)
{
    if (item->esq) // maior chave da subarvore da esquerda
    {
        return maximoRecursivo(item->esq);
    }
    else
    {
        return NULL;
    ...
    if (!vazia(arvore))
    {
        ...
        if (atual->chave == x) // encontrou chave x, tem que remover
        {
            ...
            else
            {
                ...
                Item *ante = antecessor(atual);
                Item *neto = ante->esq; //ante nao tem filho esquerdo
                // remove "suc"
                ajustaPonteiros(arvore, ante, neto);
                //substitui atual <-> suc
                //filhos de "suc"
                ante->esq = atual->esq;
                ante->dir = atual->dir;
                ...
                //pai
                ajustaPonteiros(arvore, atual, ante);
            }
            arvore->quantidade--;
            free(atual);
        }
