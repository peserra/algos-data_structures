...
{
    Item *atual = i;
    if(atual->esq != NULL) //é só ir andando até encontrar o ultimo item à direita
    {
        minimo(atual->esq);
    }
    else
...
Item *menorChave (Arvore *arv)
{
    if(!vazia(arv))
    {
        return minimo(arv->raiz);
    }
}
